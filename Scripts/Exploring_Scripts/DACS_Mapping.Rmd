---
title: "DACS"
author: "Jess"
date: '2022-12-07'
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
rm(list=ls())
require(knitr)
opts_knit$set(root.dir="~/DTSC/Manufacturing_Projects/csv/DACS")
library(raster)
library(tidyverse)
library(sf)
library(ggplot2)
library(lubridate)
library(ncdf4)
library(tidyr)
library(plotly)
library(dplyr)
library(rgdal)
```


```{r}
tracts <- st_read("CES4_Final_Shapefile.shp")
st_crs(tracts)
```

```{r}
th=theme_bw(base_size=10)+theme(strip.background= element_rect(fill="white"))
tracts_plot<-ggplot(data=tracts)+ geom_sf(data=tracts, inherit.aes = FALSE, col="#6f94e2") + th
tracts_plot
```

```{r}
dacs <- st_read("SB535DACgdb_F_2022.gdb")
ogrListLayers("SB535DACgdb_F_2022.gdb")

tribal <- readOGR("SB535DACgdb_F_2022.gdb", layer="SB535tribalboundaries2022")
st_crs(tribal)
dac_tracts <- readOGR("SB535DACgdb_F_2022.gdb", layer="SB535tracts2022")
st_crs(dac_tracts)
class(dac_tracts)
dac_tracts <- st_transform(dac_tracts, st_crs(tribal))
```
```{r}
plot(tribal)
```
```{r}
plot(dac_tracts)
```
```{r}
sb535_plot<-ggplot(data=tracts)+ geom_sf(data=tracts, inherit.aes = FALSE, col="#6f94e2") + th + geom_sf(data=tribal, inherit.aes=FALSE, fill="#2ca25f")
sb535_plot
```


```{r}
#https://www.redfoxgis.com/single-post/2015/06/20/how-to-read-an-esri-file-geodatabase-feature-class-in-r
#https://mgimond.github.io/Spatial/reading-and-writing-spatial-data-in-r.html
#https://workshops.distancesampling.org/duke-spatial-2015/practicals/process-geodata-2.html
st_layers(dacs)
```

